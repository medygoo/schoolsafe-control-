...
it("rejects blocked status in schema", async () => {
const app = await makeApp(db);
const create = await app.inject({
method: "POST",
url: "/instances",
...
// Directly try to update to blocked status, which should fail due to CHECK constraint
await expect(db.updateInstance(id, { status: "blocked" as any })).rejects.toThrow();
});
});
